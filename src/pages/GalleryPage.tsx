            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {paginatedItems.map((item, idx) => {
                ...
                return (
                  <div
                    key={item._id}
                    onClick={() => setSelectedItemIndex(realIdx)}
          ...
          {totalPages > 1 && (
            <div className="bg-white p-5 rounded-2xl border border-stone-200 shadow-luxury flex flex-col sm:flex-row items-center justify-between gap-4">
              <p className="text-xs font-semibold text-charcoal-600">
                Showing <span className="font-extrabold text-forest-950">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="font-extrabold text-forest-950">{Math.min(currentPage * itemsPerPage, items.length)}</span> of <span className="font-extrabold text-forest-950">{items.length}</span> media items
              </p>
